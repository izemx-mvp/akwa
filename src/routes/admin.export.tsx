...
const docs = [
  { name: "Commercial Invoice", status: "ok" },
  { name: "Packing List", status: "ok" },
  { name: "Bill of Lading", status: "ok" },
  { name: "Certificate of Origin", status: "missing" },
  { name: "Customs Declaration", status: "warning" },
  { name: "Insurance Certificate", status: "ok" },
];
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6 max-w-[1500px]">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-3">Export operations <AgentBadge name="Export Assistant" icon={FileCheck} /></h1>
          <p className="text-sm text-muted-foreground">Documents, customs and shipment compliance.</p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card shadow-card p-5">
          <h3 className="font-semibold mb-4">Document checklist — AKW-2410-0185</h3>
      ...
      <aside className="space-y-3">
        <div className="rounded-xl bg-gradient-ai text-ai-foreground p-5 shadow-ai sticky top-20">
          ...
          <h3 className="mt-3 text-base font-semibold">Action required</h3>
          <ul className="mt-3 text-xs text-white/85 space-y-2">
            <li>• Missing Certificate of Origin on AKW-2410-0185</li>
            <li>• Mauritania requires HS code 2710 for lubricants</li>
            <li>• Insurance certificate expires in 12 days</li>
          </ul>
        </div>
      </aside>
    </div>

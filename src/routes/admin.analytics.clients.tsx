  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <KpiTile label="CA moyen / client" value={eurCompact(d.cur.clients ? d.cur.revenue / d.cur.clients : 0)} />
        ...
        <KpiTile label="Clients à réactiver" value={num(d.inactive.length)} accent={d.inactive.length ? "warning" : "success"} />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Section className="xl:col-span-2" title="Matrice CA × marge" description="Chaque bulle est un client — taille proportionnelle au nombre de commandes">
          <Scatter points={rows.map((r) => ({ id: r.id, label: r.name, x: r.revenue, y: r.marginPct, size: r.orders }))} onPoint={(p) => setSel(p.id)} thresholds={d.thresholds} />
        </Section>
        <Section title="Concentration du portefeuille" description="Dépendance commerciale AKWA">
          <div className="grid grid-cols-2 gap-2">
            <MiniStat label="Top 1" value={pct1(d.conc.top1)} />
            <MiniStat label="Top 5" value={pct1(d.conc.top5)} />
            <MiniStat label="Top 10" value={pct1(d.conc.top10)} />
            <MiniStat label="Autres" value={pct1(100 - d.conc.top10)} />
          </div>
      ...
      <Section title="Portefeuille clients" description="Volume, rentabilité, fidélité et encours — cliquez pour ouvrir la fiche">
        <DataTable
          ...
          onRow={(r) => setSel(r.id)}
          columns={[
            { key: "n", label: "Client", render: (r) => <span className="font-medium">{r.name}</span> },
            { key: "p", label: "Pays", render: (r) => r.country },
            { key: "s", label: "Segment", render: (r) => <Tag tone="info">{r.segment}</Tag> },
            ...
            { key: "f", label: "Fréquence", align: "right", render: (r) => `${r.frequency.toFixed(1).replace(".", ",")}/mois` },
            { key: "d", label: "Dernière cde", align: "right", render: (r) => `${r.daysSince} j` },
            { key: "e", label: "Encours", align: "right", render: (r) => eur(r.outstanding), sort: (r) => r.outstanding },
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Section title="Clients à réactiver" description="Aucune commande récente malgré un historique">
          <DataTable
            rows={d.inactive}
            columns={[
              { key: "n", label: "Client", render: (r) => r.name },
              { key: "d", label: "Inactivité", align: "right", render: (r) => `${r.daysSince} j` },
      ...
      <DrillSheet open={!!cur} onOpenChange={(v) => !v && setSel(null)} title={cur?.name ?? ""} description={cur ? `${cur.country} · ${cur.segment} · ${cur.commercial}` : ""}>
        {cur && (
          <>
            <div className="grid grid-cols-3 gap-2">
              ...
              <MiniStat label="Encours" value={eur(cur.outstanding)} tone={cur.outstanding ? "text-warning" : undefined} />
            </div>
            <Section title="Commandes">
              <DataTable
                rows={d.sales.filter((s) => s.clientId === cur.id)}
                columns={[
            ...
            <Section title="Produits achetés">
              <DataTable
                rows={Object.values(
                  d.sales.filter((s) => s.clientId === cur.id).flatMap((s) => s.lines).reduce((acc, l) => {
